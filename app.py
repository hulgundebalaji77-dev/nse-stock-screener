...
st.set_page_config(page_title="Custom Indicator Signal Screener & SMS Alert", layout="wide")
...
ALL_STOCKS = [
    ...
    # Pharma & Healthcare
    "SUNPHARMA.NS", "DRREDDY.NS", "CIPLA.NS", "DIVISLAB.NS", "LUPIN.NS", "APOLLOHOSP.NS", "AUROPHARMA.NS",
    # FMCG & Consumption
    "ITC.NS", "HINDUNILVR.NS", "NESTLEIND.NS", "BRITANNIA.NS", "TATACONSUM.NS", "VBL.NS", "TITAN.NS", "DABUR.NS",
    # Metal & Energy
    ...
    # Infra & Realty
    "LT.NS", "ULTRACEMCO.NS", "GRASIM.NS", "DLF.NS", "GODREJPROP.NS", "AMBUJACEM.NS"
]

# ================= 3. SMS FUNCTION (Fast2SMS) =================
...
# ================= 5. FILTER FUNCTION (ONLY SIGNAL STOCKS) =================
def evaluate_stock_signals(df, active_filters):
    ...
    # 2. 50/200 EMA Cross (Golden / Death Cross)
    if active_filters.get('ema_50_200'):
        ...
        if p_e50 <= p_e200 and c_e50 > c_e200:
            triggers.append("🚀 50/200 EMA Golden Cross (Major Bullish)")
        elif p_e50 >= p_e200 and c_e50 < c_e200:
            triggers.append("🔻 50/200 EMA Death Cross (Major Bearish)")
    ...
    # 8. 20-Day Support / Resistance Breakout
    ...
    # फक्त आणि फक्त सिग्नल असेल तरच डेटा पाठवा
    if triggers:
...
def run_auto_background_engine(api_key, phone_no, filters, tf):
    while st.session_state.auto_scan_active:
        ...
        for sym in ALL_STOCKS:
            if not st.session_state.auto_scan_active:
                break
            try:
                df = yf.download(sym, period="5d", interval=tf, progress=False)
                if isinstance(df.columns, pd.MultiIndex):
                    df.columns = [col[0] for col in df.columns]

                res = evaluate_stock_signals(df, filters)
                if res:
                    today = datetime.now().strftime("%Y-%m-%d")
                    alert_key = f"{sym}{today}{res['Raw_Signals'][0]}"

                    if alert_key not in st.session_state.sent_alerts:
                        sms_msg = f"SIGNAL ALERT:\n{sym}\nPrice: ₹{res['Close']:.2f}\nRSI: {res['RSI']:.1f}\nTrigger: {res['Signals']}"
                        sms_res = send_sms_alert(phone_no, sms_msg, api_key)
                        
                        if sms_res.get("return"):
                            st.session_state.sent_alerts.add(alert_key)

                        st.session_state.live_auto_table.insert(0, {
                            "वेळ (Time)": now_time,
                            "स्टॉक (Stock)": sym,
...
# ================= 7. LEFT SIDEBAR (ALL INDICATOR FILTERS) =================
st.sidebar.title("🎛️ इंडिकेटर्स फिल्टर्स (Left Menu)")
st.sidebar.caption("ज्या इंडिकेटर्सचे सिग्नल्स हवे आहेत ते सिलेक्ट करा:")
...
active_filters = {
    ...
    "rsi": st.sidebar.checkbox("RSI (Oversold < 30 / Overbought > 70)", value=True),
    ...
    "ema_50_200": st.sidebar.checkbox("50/200 EMA (Golden / Death Cross)", value=False),
    "sr_breakout": st.sidebar.checkbox("20-Day Support/Resistance Breakout", value=False),
    "stoch_rsi": st.sidebar.checkbox("Stochastic RSI (20 / 80 Extremes)", value=False),
}

st.sidebar.markdown("---")
timeframe = st.sidebar.selectbox("टाइमफ्रेम (Timeframe):", ["15m", "5m", "1h", "1d"], index=0)
...
if st.sidebar.button("🚀 ऑटो स्कॅनर सुरू करा"):
    if api_key:
        if not st.session_state.auto_scan_active:
            ...
            threading.Thread(target=run_auto_background_engine, args=(api_key, phone_no, active_filters, timeframe), daemon=True).start()
...
if st.sidebar.button("⏹️ ऑटो स्कॅनर थांबवा"):
    st.session_state.auto_scan_active = False
    st.sidebar.warning("ऑटो स्कॅनर बंद केले.")

# ================= 8. MAIN DASHBOARD =================
st.title("🎯 केवळ सिग्नल देणारे शेअर्स (Signal-Only Scanner)")
st.caption("डाव्या बाजूला निवडलेल्या इंडिकेटर्सनुसार ज्या शेअर्समध्ये *Buy/Sell/Breakout ट्रिगर* झाला आहे, फक्त तेच शेअर्स येथे दिसतील.")

tab1, tab2 = st.tabs(["⚡ त्वरित स्कॅन निकाल (Filtered Signals)", "📈 सिलेक्टेड शेअरचा लाईव्ह चार्ट"])
...
with tab1:
    ...
    if scan_btn:
        with st.spinner("निवडलेल्या इंडिकेटर्सनुसार सर्व शेअर्स स्कॅन होत आहेत..."):
            ...
            if matched_stocks:
                st.success(f"🎯 एकूण {len(matched_stocks)} शेअर्समध्ये निवडलेल्या इंडिकेटर्सचे सिग्नल्स सापडले आहेत!")
                st.dataframe(pd.DataFrame(matched_stocks), use_container_width=True)
            else:
                st.info("निवडलेल्या इंडिकेटर्सनुसार सध्या कोणत्याही शेअरमध्ये सिग्नल तयार झालेला नाही.")

    st.markdown("---")
    st.subheader("📡 ऑटो-स्कॅनरद्वारे सापडलेले शेअर्स (Live Auto SMS Triggered)")
    if st.session_state.auto_scan_active:
        st.success("🟢 बॅकग्राउंड ऑटो-स्कॅनर सुरू आहे (नवीन सिग्नल मिळताच थेट 8459958007 वर SMS जाईल).")
    else:
        st.warning("⚪ ऑटो-स्कॅनर बंद आहे. सुरू करण्यासाठी साइडबारमधील बटण दाबा.")

    if st.session_state.live_auto_table:
        st.dataframe(pd.DataFrame(st.session_state.live_auto_table), use_container_width=True)

with tab2:
    st.subheader("📈 कोणत्याही शेअरचा सविस्तर कँडलस्टिक चार्ट")
    chart_ticker = st.selectbox("चार्ट पाहण्यासाठी स्टॉक निवडा:", ALL_STOCKS)
    ...
    if not df_chart.empty:
        ...
        fig.add_trace(go.Scatter(x=df_chart.index, y=df_chart['EMA_9'], line=dict(color='blue', width=1.2), name="9 EMA"), row=1, col=1)
        fig.add_trace(go.Scatter(x=df_chart.index, y=df_chart['EMA_21'], line=dict(color='orange', width=1.2), name="21 EMA"), row=1, col=1)
        fig.add_trace(go.Scatter(x=df_chart.index, y=df_chart['EMA_200'], line=dict(color='red', width=1.5), name="200 EMA"), row=1, col=1)
        fig.add_trace(go.Scatter(x=df_chart.index, y=df_chart['BB_High'], line=dict(color='gray', dash='dash'), name="BB Upper"), row=1, col=1)
        fig.add_trace(go.Scatter(x=df_chart.index, y=df_chart['BB_Low'], line=dict(color='gray', dash='dash'), fill='tonexty', fillcolor='rgba(200,200,200,0.1)', name="BB Lower"), row=1, col=1)

        # Panel 2: Volume
        vol_col = ['green' if c >= o else 'red' for c, o in zip(df_chart['Close'], df_chart['Open'])]
        fig.add_trace(go.Bar(x=df_chart.index, y=df_chart['Volume'], marker_color=vol_col, name="Volume"), row=2, col=1)

        # Panel 3: RSI & MACD
        fig.add_trace(go.Scatter(x=df_chart.index, y=df_chart['RSI'], line=dict(color='purple', width=1.3), name="RSI (14)"), row=3, col=1)
